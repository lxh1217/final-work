		...
		def _get_feed_dict(self, index):
			...
			if self.phase == 'train':
				...
				feed_dict = {
					...
					'item_id': [self.data['item_id'][index]],  # 正样本
				}
				
				# 生成负样本
				num_neg = getattr(self.model.args, 'num_neg', 1)
				neg_items = []
				
				for _ in range(num_neg):
					# 随机选择负样本，确保不是正样本
					neg_item = np.random.randint(0, self.model.item_num)
					# 简单的去重（在实际应用中可能需要更复杂的逻辑）
					while neg_item == self.data['item_id'][index]:
						neg_item = np.random.randint(0, self.model.item_num)
					neg_items.append(neg_item)
				
				# 将正样本和负样本合并
				feed_dict['item_id'] = [self.data['item_id'][index]] + neg_items
			...
			if self.phase == 'train':
				...
				if 'neg_items' in self.data:
					...
					if isinstance(neg_items, list):
						# 已经是列表形式
						neg_items_list = neg_items
					else:
						# 可能是numpy数组
						neg_items_list = neg_items.tolist()
				else:
					# 生成负样本
					num_neg = getattr(self.model, 'num_neg', 1)
					neg_items_list = []
				...
				# 合并正负样本
				feed_dict['item_id'] = [pos_item] + neg_items_list
				
			else:
				# 测试/验证数据：默认只有一个正样本
				# 注意：具体的负样本生成应该在子类中实现
				feed_dict['item_id'] = [pos_item]
	...
	class Dataset(GeneralModel.Dataset):
		...
		def _get_feed_dict(self, index):
			...
			if self.phase == 'train':
				...
				# 生成负样本
				num_neg = self.model.args.num_neg if hasattr(self.model.args, 'num_neg') else 1
				neg_items = []
